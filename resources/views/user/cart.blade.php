   ...
   <div class="container-fluid">
    <div class="row px-xl-5">
        ...
        <div class="col-lg-8 table-responsive mb-5">
                    ...
                    @foreach ($cartItems as $item)
                        <tr>
                            <td class="align-middle">
                               <img src="{{ asset('images/' . $item->product->gambar) }}" width="80" alt="{{ $item->product->nama_produk }}">
                            </td>
                            ...
                            <td class="align-middle">
                               <form action="{{ route('cart.remove', $item->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini dari cart?');">
                                @csrf
                                @method('DELETE')
            ...
            {{-- Hidden input untuk pengecekan stok --}}
            @foreach ($cartItems as $item)
            <input type="hidden" class="stok-item"
            ...
            <div class="bg-light p-30 mb-5">
                ...
                <div class="pt-2">
                    ...
                    <div class="modal-dialog">
                        ...
                        <div class="modal-body" id="stokModalBody">
                            <!-- Akan diisi JS -->
                        </div>
                        <div class="modal-footer">
                        <a href="{{ url('cart') }}" class="btn btn-secondary">Perbaiki Keranjang</a>
                        </div>
                        </div>
                    </div>
                    </div>

                   <button id="btnCheckout" class="btn btn-block btn-primary font-weight-bold my-3 py-3">
                    Proceed To Checkout
                </button>
                </div>
            </div>
...
document.getElementById('btnCheckout').addEventListener('click', function (e) {
    ...
    items.forEach(item => {
        ...
        if (qty > stok) {
            stokKurang.push(`- ${nama}: dipesan ${qty}, stok hanya ${stok}`);
        }
    });
    ...
    if (stokKurang.length > 0) {
        ...
        document.getElementById('stokModalBody').innerHTML =
            `<p>Beberapa barang tidak mencukupi stok:</p><ul>` +
            stokKurang.map(e => `<li>${e}</li>`).join('') +
            `</ul>`;
        new bootstrap.Modal(document.getElementById('modalStokKurang')).show();
    } else {
        // Lanjut ke checkout
